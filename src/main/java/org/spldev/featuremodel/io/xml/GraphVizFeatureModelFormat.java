...
public class GraphVizFeatureModelFormat implements Format<FeatureModel> {
    ...
    public String serialize(FeatureModel featureModel) {
        ...
        return String.format("digraph {\n  graph%s;\n  node%s;\n  edge%s;\n%s\n%s\n}",
                options(option("splines", "false"),
                        option("ranksep", "0.2")),
                options(option("fontname", "Arial"),
                        option("style", "filled"),
    ...
    public String getNode(Feature feature) {
        String nodeString = "";
        nodeString += String.format("  %s%s;",
                quote(feature.getIdentifier().toString()),
                options(option("label", feature.getName()),
                        option("fillcolor", feature.isAbstract() ? "#f2f2ff" : null)));
        nodeString += String.format("\n  %s%s;",
                quote(feature.getIdentifier().toString() + "_group"),
                options(option("shape", "diamond"),
                        option("style", !feature.getFeatureTree().isGroup() ? "invis" : feature.getFeatureTree().isAlternative() ? "" : null),
    ...
    public String getEdge(Feature feature) {
        ...
        if (feature.getFeatureTree().hasParent()) {
            String parentNode = feature.getFeatureTree().getParent().get().getFeature().getIdentifier().toString();
            edgeString += getEdge(parentNode + "_group", feature,
                    option("style", feature.getFeatureTree().getParent().get().isGroup() ? null : "invis"));
            if (!feature.getFeatureTree().getParent().get().isGroup())
                edgeString += getEdge(parentNode + (feature.getFeatureTree().getParent().get().isGroup() ? "_group" : ""), feature, "");
        }
        edgeString += String.format("  %s:s -> %s:n%s;",
                quote(feature.getIdentifier().toString()),
                quote(feature.getIdentifier().toString() + "_group"),
    ...
    public String getEdge(String parentNode, Feature childFeature, String option) {
        return String.format("  %s:s -> %s:n%s;\n",
                    ...
                    options(option("arrowhead",
                            childFeature.getFeatureTree().getParent().get().isGroup() ? null :
    ...
    protected String options(String... options) {
        List<String> optionsList = Arrays.stream(options).filter(o -> !o.isEmpty()).collect(Collectors.toList());
        if (String.join("", optionsList).trim().isEmpty())
            return "";
