...
public class RenamingsManager implements IEventManager, Cloneable {
	 ...
	 * @param oldModel
	 */
	protected RenamingsManager(RenamingsManager renamingsManager, FeatureModel newModel) {
		model = newModel;
		renamings.addAll(renamingsManager.renamings);
	}
	...
	public boolean renameFeature(final String oldName, final String newName) {
		...
		final Feature feature = model.getFeature(oldName);
		model.deleteFeatureFromTable(feature);
		feature.setName(newName);
		model.addFeature(feature);
		renamings.add(new Renaming(oldName, newName));
		for (final Constraint c : constraints) {
			// renameVariables(c.getNode(), oldName, newName); todo
		}
...
//		final FeatureModelManager instance = FeatureModelManager.getInstance(model);
...
//	private void renameVariables(Formula node, String oldName, String newName) {
...
//	}
	...
	/**
	 * Returns the name of a feature at the time of the last save given its current
	 * name.
	 *
	...
	public RenamingsManager clone(FeatureModel newModel) {
