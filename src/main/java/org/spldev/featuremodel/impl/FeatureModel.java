...
import org.spldev.featuremodel.FeatureUtils;
...
import org.spldev.featuremodel.IFeatureModelStructure;
...
import org.spldev.featuremodel.event.FeatureIDEEvent.EventType;
...
public class FeatureModel implements IFeatureModel {
	...
	protected FeatureModel(FeatureModel oldFeatureModel, IFeature newRoot) {
		...
		if (newRoot == null) {
			...
			for (final IConstraint constraint : oldFeatureModel.constraints) {
				if (featureTable.keySet().containsAll(constraint.getContainedFeatures().stream().map(IFeature::getName).collect(Collectors.toList()))) {
					constraints.add(constraint.clone(this));
				}
			}
		}
